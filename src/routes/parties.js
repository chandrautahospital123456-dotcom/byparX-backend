...
router.get('/customers', async (req, res, next) => {
  try {
    ...
    let q = db('parties').where({ company_id: req.companyId, type: 'customer' })
    if (search) q = q.where(b => b.whereILike('name', `%${search}%`).orWhereILike('phone', `%${search}%`).orWhereILike('code', `%${search}%`))
...
router.get('/suppliers', async (req, res, next) => {
  try {
    const { page, limit, offset } = parsePagination(req.query)
    const { search } = req.query
    let q = db('parties').where({ company_id: req.companyId, type: 'supplier' })
    if (search) q = q.where(b => b.whereILike('name', `%${search}%`).orWhereILike('phone', `%${search}%`).orWhereILike('code', `%${search}%`))
    const [{ count }] = await q.clone().count('id as count')
    const data = await q.orderBy('name').limit(limit).offset(offset)
    return paginatedResponse(res, { data, total: Number(count), page, limit })
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const allowed = ['name','phone','email','address','pan_no','credit_limit','credit_days','opening_balance','is_active']
    const updates = {}
