...
router.post('/', async (req, res, next) => {
  ...
  try {
    ...
    // L143-145 FIX: accounting_accounts → accounts; accounting_entries INSERT removed
    // Full double-entry is handled by postingEngine separately.
    // We do a lightweight cash account lookup for future integration only.
    if (payment_mode !== 'credit') {
      try {
        await trx('accounts')                                    // L143 FIX: was 'accounting_accounts'
          .where({ company_id: req.companyId, sub_type: 'cash' }).first()
        // L145 FIX: trx('accounting_entries').insert() — REMOVED (ghost table)
        // postingEngine creates the proper voucher + journal_entries asynchronously
      } catch { /* non-critical */ }
    }

    await trx.commit()
    auditLog(req.companyId, req.user.id, 'CREATE', 'sales', sale.id, { invoice_no, net_total }, req.ip)
    return successResponse(res, { ...sale, items: saleItems }, 'Invoice created', 201)
